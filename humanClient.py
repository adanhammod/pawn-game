...
global color_turn
color_turn = "white"  # Default or initial value
running =True
...
def update_timer():
    ...
    global white_time_remaining, black_time_remaining, last_time_update
...
def is_legal_move(board, start_row, start_col, end_row, end_col, piece):
    """Validate if the pawn's move is legal."""
    if piece == "":
        return False  # No piece at the start square
    ...
    # Pawn movement logic
    direction = -1 if piece == "P_white" else 1  # White moves up, Black moves down
    starting_row = 6 if piece == "P_white" else 1  # Starting row for the piece
    

    # Check for one-square movement
    if start_col == end_col and start_row + direction == end_row:
        if board[end_row][end_col] == "":  # Destination must be empty
            return True

    # Check for two-square move from the starting row
    if start_row == starting_row and start_col == end_col and abs(start_row - end_row) == 2:
        intermediate_row = start_row + direction
        if board[end_row][end_col] == "" and board[intermediate_row][start_col] == "":
            return True

    # Check for pawn capture (diagonal move)
    if abs(start_col - end_col) == 1 and start_row + direction == end_row:
        if board[end_row][end_col] != "":  # Captures require a piece at the destination
            return True
...
def get_user_move():
    ...
    while running:  # Keep prompting until a valid move is entered
        move = input("Enter your move (e.g., e2e4): ").strip().lower()
        
        # Check input format using regex
        if not re.match(r"^[a-h][1-8][a-h][1-8]$", move):
            print("Invalid input format. Please enter a valid move (e.g., e2e4).")
            continue
        ...
        piece = board[start_row][start_col]  # Get the piece at the start position
        
        # Ensure there's a piece at the start square and it matches the player's turn
        if not piece or not piece.endswith(color):
            print("No valid piece at the start square or not your turn. Try again.")
            continue

        # Validate the move using the legal move function
        if is_legal_move(board, start_row, start_col, end_row, end_col, piece):
            ...
            if screen is not None and running:
              ...
              return True  # Exit the loop once a valid move is made
        else:
...
def handle_socket_communication(socketObject):
    ...
    global color, board, game_started, color_turn, running, time_per_turn_minutes, time_per_turn_seconds, white_time_remaining
    ...
    while running:
        ...
        if data.startswith("Time"):
            ...
            if winner and running:
                ...
                get_user_move()  # User input for white
                if(get_moves_history()):
                    ai_last_move = get_moves_history()[-1]
                    if ai_last_move and running:
...
# Pygame event loop
while running:
    update_timer()  # Update the timer at the start of each loop iteration
